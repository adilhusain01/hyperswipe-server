...
class HyperliquidWebSocketManager:
    ...
    async def handle_user_events_for_telegram(self, user_address: str, data):
        """Handle user events - fill notifications now handled by FillNotificationService"""
        logger.debug(f"📊 UserEvents received for {user_address} - fill notifications handled by API polling service")
        # Fill notifications are now handled by the dedicated FillNotificationService
        # This method is kept for future user event handling if needed
